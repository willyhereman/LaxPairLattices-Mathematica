...
(* if delta = 0 then equation has no rhs, called Q2zero *)
(* if delta =!= 0 then equation has rhs, called Q2rhs *)
(* delta = 0; *)

(* lattice[origin_,right_,across_,up_,parameterright_,parameterup_] *)

name = Q2rhs;

lattice[x_,x1_,x2_,x12_,p_,q_]:= 
  p*(x-x2)*(x1-x12)-q*(x-x1)*(x2-x12)+delta*p*q*(p-q)*(x+x1+x2+x12)-
  delta^2*p*q*(p-q)*(p^2-p*q+q^2) == 0;
...
(* may not work correctly for complicated lattices, in particular, if roots *)
